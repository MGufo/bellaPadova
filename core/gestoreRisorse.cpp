...
bool GestoreRisorse::controlloInInventario(Articolo* daControllare) const {
  bool presente = true;
  const Lista<const Consumabile*>* consumabili =
      daControllare->getComposizione();
  for (auto it = consumabili->const_begin();
       (it != consumabili->const_end()) && presente; ++it) {
    for (auto it = inventario.const_begin();
         it != inventario.const_end() && !presente; ++it)
      if ((*it)->getIdRisorsa() == daControllare->getIdRisorsa())
        presente = true;
  }
  delete consumabili;
  return presente;
...
bool GestoreRisorse::controlloConsumabile(
  ...
  bool trovato = false;
  for (auto it = _lista->const_begin(); it != _lista->const_end() && !trovato;
       ++it)
  ...
  if (daInserire) {
    ...
    if (!controlloInInventario(daInserire))
      throw new std::logic_error(
          "Errore: uno o più elementi della composizione di questo articolo "
          "non sono attualmente disponibili.");
    for (auto it = menu.const_begin(); it != menu.const_end(); ++it) {
      if (daInserire->getIdRisorsa() == (*it)->getIdRisorsa())
        throw new std::domain_error(
            "Errore: l'articolo è già presente nel menù.");
    }
  ...
  if (daRimuovere) {
    for(auto it = menu.begin(); it != menu.end(); ++it) {
      ...
      if (controlloConsumabile(lista, daRimuovere))
        (*it)->setDisponibilita(false);
      delete lista;
    }
    inventario.erase(inventario.find(daRimuovere));
    // casi particolari in cui daRimuovere è sia Consumabile che Articolo,
    // es:Bevanda
    Articolo* temp =
        dynamic_cast<Articolo*>(static_cast<Risorsa*>(daRimuovere));
    if (temp) {
      bool trovato = false;
...
void GestoreRisorse::modificaArticolo(Articolo* daModificare,
                                      Articolo* modificato) {
  const Lista<const Consumabile*>* composizioneDaModificare =
      daModificare->getComposizione();
  if (modificato->getDisponibilita() &&
      !controlloDisponibilita(composizioneDaModificare))
    throw new std::logic_error(
        "Errore: Impossibile modificare l'elemento in quanto uno o più "
        "elementi della sua composizione non sono disponibili.");
  for (auto it = menu.begin(); it != menu.end(); ++it) {
    if (daModificare->getIdRisorsa() == (*it)->getIdRisorsa())
      (*it)->modifica(modificato);
  }
  daModificare->setDisponibilita(
      daModificare->getDisponibilita() &&
      controlloDisponibilita(composizioneDaModificare));
...
void GestoreRisorse::modificaConsumabile(Consumabile* daModificare,
  ...
  for (auto it = menu.begin(); it != menu.end(); ++it) {
    const Lista<const Consumabile*>* lista = (*it)->getComposizione();
    (*it)->setDisponibilita((*it)->getDisponibilita() &&
                            controlloDisponibilita(lista));
    delete lista;
  }
}
...
void GestoreRisorse::salvaRisorse(QJsonObject* risorseJSON) const {
  ...
  for (auto it = inventario.const_begin(); it != inventario.const_end(); ++it) {
    ...
    inventarioJSON->insert(QString::fromStdString((*it)->getNome()),
                           *consumabileJSON);
  ...
  for (auto it = menuJSON.constBegin(); it != menuJSON.constEnd(); ++it) {
    ...
    if (tipo == "pizza") {
      ...
      for (auto it = ingrJSON->constBegin(); it != ingrJSON->constEnd(); ++it)
        (*keymap)[(*it).toInt()] = trovaRisorsa((*it).toInt());
      delete ingrJSON;
    }
    if (risorsa) {
      ...
      for (auto it = inventario.const_begin(); it != inventario.const_end();
           ++it) {
        if ((*it)->getIdRisorsa() == id)
          bevandaInMenu = dynamic_cast<Articolo*>(static_cast<Risorsa*>(*it));
      }
...
void GestoreRisorse::caricaInventario(const QJsonObject& risorseJSON) {
  ...
  for (auto it = risorseJSON.constBegin(); it != risorseJSON.constEnd(); ++it) {
    ...
    if (tipo == "bottiglia")
      risorsa = new Bottiglia();
    else if (tipo == "lattina")
      risorsa = new Lattina();
    else if (tipo == "ingrediente")
      risorsa = new Ingrediente();
    else if (tipo == "farina")
      risorsa = new Farina();
