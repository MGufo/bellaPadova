...
double Pizzeria::calcoloGuadagno(const QJsonObject& comandeJSON,
  ...
  for (auto it = comandeJSON.constBegin(); it != comandeJSON.constEnd(); ++it) {
    QDate dataConsegna = QDate::fromString(
        (*(*it).toObject().constFind("dataConsegna")).toString());
...
double Pizzeria::contabilizzazione(const QDate& inizio,
                                   const QDate& fine) const {
  ...
  for (auto it = consumabili->const_begin(); it != consumabili->const_end();
       ++it) {
    if ((inizio <= (*it)->getDataAcquisto()) &&
        ((*it)->getDataAcquisto() <= fine)) {
      costi += (*it)->getSpesa();
    }
  }
...
void Pizzeria::modificaArticolo(Articolo* daModificare, Articolo* modificato) {
...
void Pizzeria::rimuoviArticolo(Articolo* daRimuovere) {
  ...
  if (!dynamic_cast<Bevanda*>(daRimuovere)) delete daRimuovere;
}
...
void Pizzeria::inserisciComanda(Comanda* daInserire) {
  if (daInserire) {
    ...
    const unordered_map<Articolo*, unsigned int>* ordinazione =
        &daInserire->getOrdinazione();
    ...
    for (it = ordinazione->cbegin(); (it != ordinazione->cend()) && inseribile;
         ++it) {
    ...
    if (inseribile) {
      ...
      std::stringstream errorMsg;
      errorMsg << "Errore: Non è possibile inserire la comanda perché uno o "
                  "più ingredienti necessari per creare l'articolo "
               << ((*it).first)->getNome() << " non sono disponibili";
      throw new std::logic_error(errorMsg.str());
    }
  }
}
...
void Pizzeria::modificaComanda(Comanda* daModificare, Comanda* modificata) {
  ...
  if (inseribile)
    ...
    errorMsg << "Errore: Non è possibile modificare la comanda perché uno o "
                "più ingredienti necessari per creare l'articolo "
             << ((*it).first)->getNome() << " non sono disponibili";
    throw new std::logic_error(errorMsg.str());
  }
...
void Pizzeria::caricaRisorse() {
  ...
  gestoreRisorse.caricaInventario(
      (*risorseJSON->constFind("Inventario")).toObject());
