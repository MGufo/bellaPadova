...
Comanda::Comanda(unsigned int _id, Contatto _cliente, QTime _oraConsegna,
                 QDate _dataConsegna,
                 unordered_map<Articolo*, unsigned int> _ordinazione, double t)
    : ID(_id),
      cliente(_cliente),
      oraConsegna(_oraConsegna),
      dataConsegna(_dataConsegna),
      ordinazione(_ordinazione),
      totale(t) {}

unsigned int Comanda::getIdComanda() const { return ID; }
...
QTime* Comanda::getOrarioInizioPreparazione(unsigned short capForno) const {
  QTime* orario = new QTime(
      getOraConsegna().addSecs(-(getTempoPreparazione(capForno) * 60)));
  return orario;
}
