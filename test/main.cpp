...
int main(int argc, char* argv[]) {
  ...
  controller.caricaRisorse();
    ...
    Farina* farro = new Farina(2, "farina di tipo farro 1kg", true, 10, 2, QDate::currentDate(), false, "farro");

    //string _nome, bool _disponibilita, unsigned int _quantita,
    //double _costo, QDate _dataAcquisto, bool _locale
    Ingrediente* mozz = new Ingrediente(3, "mozzarella", true, 10, 5, QDate::currentDate());
    Ingrediente* pomm = new Ingrediente(4, "pomodoro", true, 10, 5, QDate::currentDate(), true);
    Ingrediente* wrustel = new Ingrediente(5, "wrustel", true, 10, 5, QDate::currentDate());
    Ingrediente* funghi = new Ingrediente(6, "funghi", true, 10, 5, QDate::currentDate(), true);
    Ingrediente* ananas = new Ingrediente(7, "ananas", true, 10, 5, QDate::currentDate(), true);
    ...
    Lattina* Fanta = new Lattina(11, "Fanta", true, 2, 10, 6, QDate::currentDate(), 0.33);
    Bottiglia* CocaCola = new Bottiglia(12, "Coca Cola", true, 4, 10, 8, QDate::currentDate(), 1.5);
    ...
    Comanda* comanda1 = new Comanda(1, Contatto("AndyM", "Via Prova 123","123456789"), QTime(20,00), QDate(2020,06,17));
    comanda1->inserisciArticolo(marghe,2);
    comanda1->inserisciArticolo(mari, 1);
    comanda1->inserisciArticolo(CocaCola, 1);

    Comanda* comanda2 = new Comanda(2, Contatto("gBizz", "Via Stocazzo","987654321"), QTime(20,30), QDate(2020,06,17));
